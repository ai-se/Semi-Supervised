...
svs=find(alpha~=0);
classifier=saveclassifier_1('lapsvmp',svs,alpha(svs),data.X(svs,:),b,options,sec,t,lsiters,stats);
...
while 1  
    ...
    if options.Hinge        
        sv_prev=sv;         
        hloss=sparse([],[],[],n,1,l);
        disp(size(hloss(labeled)))
        disp(size((1-data.Y(labeled).*(Kalpha(labeled,:)+b))))
