    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        try:
            if self._symmetric:
                ...
                linkm = np.array(np.dot(self._data1 , self._data2.T))
                trnorms1 = []
                for i in range(m):
                    # print(self._data1[i], self._data1[i].T)
                    # print(np.dot(self._data1[i] , self._data1[i].T))
                    trnorms1.append(np.dot(self._data1[i] , self._data1[i].T))
                trnorms1 = np.array(trnorms1).T
                trnorms1 = trnorms1.reshape((trnorms1.shape[0],1))
                trnorms2 = []
                for i in range(n):
                    trnorms2.append(np.dot(self._data2[i] , self._data2[i].T))
                trnorms2 = np.array(trnorms2).T
                trnorms2 = trnorms2.reshape((trnorms2.shape[0],1))
