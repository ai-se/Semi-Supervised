...
def load_data(project):
    train_df, test_df, success  = prepare_data_commit_guru_file(project)#,'process')
...
def prepare_data_commit_guru_file(project):
    ...
    commits = df.commit_hash
    
    file_data_path = 'data/' + project + '.csv'
    commit_data_path = 'data/' + project + '.csv'
    release_data_path = 'data/' + project + '_release.pkl'
    
    commit_data_df = pd.read_csv(commit_data_path)
    commit_data_df = commit_data_df[['commit_hash','contains_bug','author_date_unix_timestamp']]
    commit_data_df['contains_bug'].fillna(False, inplace = True)
    commit_data_df["contains_bug"] = commit_data_df["contains_bug"].astype(int)
    commit_data_df['created_at'] = pd.to_datetime(commit_data_df.author_date_unix_timestamp,unit='s')
    commit_data_df['created_at'] = commit_data_df.created_at.dt.date
    commit_data_df = commit_data_df[['commit_hash','created_at','contains_bug']]
    ...
    release_df = pd.read_pickle(release_data_path)
    ...
    data_df = data_df.sort_values(by = 'created_at', ascending=True)
    ...
    created_at = data_df.created_at
    y = data_df.contains_bug
    X = data_df.drop(['contains_bug','created_at'],axis = 1)
    cols = X.columns
    scaler = MinMaxScaler()
    ...
    data_df['created_at'] = created_at.values.tolist()
    data_df['commit_hash'] = commit_hash.values.tolist()
    ...
    for release_date in release_df.created_at.unique()[:-1]:
        test_df =  data_df[(data_df.created_at >= release_date)]
        if (test_df.shape[0]) > 2 and (test_df.shape[0] > test_size) and (len(test_df.Bugs.unique()) == 2):
            count += 1
    ...
    if count < 4:
        ...
        return data_df,data_df,0

    train_df =  data_df[data_df.created_at < last_train_date]
    test_df =  data_df[data_df.created_at >= last_train_date]
...
def lapSvmp(X_train_mixedlabeled, y_train_mixedlabeled, X_test,y_test):
    ...
    eng.addpath(r'lapsvmp/',nargout=0)
...
get_data
'''
def get_data(train_df,X_test,y_test, loc_data, percentage):
    training_size = train_df.shape[0]
    frac = percentage/100
    labeled_data_size = int(training_size*frac)

    y_train_supervised = train_df.Bugs
    X_train_supervised = train_df.drop(['Bugs'], axis = 1)

    train_df_bugs = train_df[train_df['Bugs'] == 1]
    train_df_non_bugs = train_df[train_df['Bugs'] != 1] 

    train_df_bugs.reset_index(drop = True, inplace = True)
    train_df_non_bugs.reset_index(drop = True, inplace = True)

    final_train_df = pd.DataFrame()

    for i in range(train_df_bugs.shape[0]):
        if i <= int(labeled_data_size/2):
            train_df_bugs.loc[i,'Bugs'] = train_df_bugs.loc[i,'Bugs']
        else:
            train_df_bugs.loc[i,'Bugs'] = -1

    for i in range(train_df_non_bugs.shape[0]):
        if i <= int(labeled_data_size/2):
            train_df_non_bugs.loc[i,'Bugs'] = train_df_non_bugs.loc[i,'Bugs']
        else:
            train_df_non_bugs.loc[i,'Bugs'] = -1

    final_train_df = copy.deepcopy(train_df_bugs)
    final_train_df = pd.concat([final_train_df, train_df_non_bugs], axis = 0)



    train_df = final_train_df
    unlabeled_y = train_df.Bugs.values.tolist() 

    train_df = train_df.drop(['LOC','commit_hash'], axis = 1)
    

    mixed_train_df = copy.deepcopy(train_df)
    ...
    X_train, X_val, y_train, y_val = train_test_split(X_train_labeled, y_train_labeled, test_size=0.20, random_state=42)
    ...
    # print('after', X_train_labeled.shape)
...
def compile_results(y_test, predicted, loc, results, _error):
    y_test = y_test.values.tolist()
    if _error:
        results['f1'].append(0)
        results['precision'].append(0)
        results['recall'].append(0)
        results['g-score'].append(0)
        results['d2h'].append(0)
        results['pci_20'].append(0)
        results['ifa'].append(0)
        results['pd'].append(0)
        results['pf'].append(0)
        return results
...
def run_classifiers(project, supervised_model_list, percentage):

    train_df,test_df,success  = load_data(project)

    perc = {2.5:'2_5', 5: '5', 10: '10', 20: '20'}

    if not success:
        print('project,',  project, ' unsuccessful')
        return 0
    else:
        print('=========='*10)
        print(project, train_df.shape, test_df.shape)
    ...
    for release in test_df.release.unique():
        ...
        _test_df = copy.deepcopy(test_df)
        _test_df = _test_df[_test_df['release'] == release]

        y_test = test_df.Bugs
        loc_data = test_df.LOC
        X_test = test_df.drop(['Bugs', 'LOC','release'], axis = 1)

        data = get_data(train_df,X_test,y_test, loc_data, percentage)
        ...
        unlabeled_df = data[9]
        effort = data[10]
        X_train_supervised = data[11]
        y_train_supervised = data[12]
        loc = loc_data[X_test.index]

        print('size:', X_train_labeled.shape)


        for model in supervised_model_list.keys():
            print('Starting a New Model', model)
            ...
            try:
                ...
                all_results[model] = compile_results(y_test, predicted, loc, all_results[model], False)
            except Exception as e:
                print(model, e)
                all_results[model] = compile_results(y_test, predicted, loc, all_results[model], True)
                continue
        ...
        for model in supervised_model_list.keys():
            try:
                ...
                if first_build:
                    ...
                    models[self_training_model] = clf
                else:
                    clf = models[self_training_model]
                predicted = clf.predict(X_test)
                all_results[self_training_model] = compile_results(y_test, predicted, loc, all_results[self_training_model], False)
            except Exception as e:
                print(model, e)
                all_results[self_training_model] = compile_results(y_test, predicted, loc, all_results[self_training_model], True)
                continue
        ...
        try:
            if first_build:
                ...
                models['LabelPropagation'] = label_prop_model
            else:
                label_prop_model = models['LabelPropagation']
            
            predicted = label_prop_model.predict(X_test)
            all_results['LabelPropagation'] = compile_results(y_test, predicted, loc, all_results['LabelPropagation'], False)
        except Exception as e:
            print(model, e)
            all_results['LabelPropagation'] = compile_results(y_test, predicted, loc, all_results['LabelPropagation'], True)
        ...
        try:
            if first_build:
                ...
                models['label_spread_model'] = label_spread_model
            else:
                label_spread_model = models['label_spread_model']  
            predicted = label_spread_model.predict(X_test)
            all_results['LabelSpreading'] = compile_results(y_test, predicted, loc, all_results['LabelSpreading'], False)
        except Exception as e:
            print(model, e)
            all_results['LabelSpreading'] = compile_results(y_test, predicted, loc, all_results['LabelSpreading'], True)
        ...
        try:
            if first_build:
                ...
                models['Semi_GMM'] = [gm, label]
            else:
                gm, label = models['Semi_GMM'][0], models['Semi_GMM'][1]
            _Xt = copy.deepcopy(X_test)
            predicted = semi_GMM_predict(gm, label, _Xt)
            all_results['Semi_GMM'] = compile_results(y_test, predicted, loc, all_results['Semi_GMM'], False)
        except Exception as e:
            print(model, e)
            all_results['Semi_GMM'] = compile_results(y_test, predicted, loc, all_results['Semi_GMM'], True)
        print("Cluster Models Done")

        for combo in combinations(supervised_model_list.keys(), 2):
            ...
            try:
                ...
                if first_build:
                    ...
                    models[co_training_sv_model] = ctc
                else:
                    ctc = models[co_training_sv_model]
                predicted = ctc.predict([X_test,X_test])
                all_results[co_training_sv_model] = compile_results(y_test, predicted, loc, all_results[co_training_sv_model], False)
            except Exception as e:
                print(model, e)
                all_results[co_training_sv_model] = compile_results(y_test, predicted, loc, all_results[co_training_sv_model], True)
                continue
        ...
        for combo in combinations(supervised_model_list.keys(), 2):
            try:
                ...
                if first_build:
                    ...
                    models[co_training_mv_model] = [ctc, view_1, view_2]
                else:
                    ctc, view_1, view_2 = models[co_training_mv_model][0], models[co_training_mv_model][1], models[co_training_mv_model][2]
                predicted = ctc.predict([X_test[view_1], X_test[view_2]])
                all_results[co_training_mv_model] = compile_results(y_test, predicted, loc, all_results[co_training_mv_model], False)
            except Exception as e:
                print(model, e)
                all_results[co_training_mv_model] = compile_results(y_test, predicted, loc, all_results[co_training_mv_model], True)
                continue
        ...
        try:
            if first_build:
                ...
                clf = models['EATT']
            predicted = clf.predict(X_test)
            all_results['EATT'] = compile_results(y_test, predicted, loc, all_results['EATT'], False)
        except Exception as e:
            print(model, e)
            all_results['EATT'] = compile_results(y_test, predicted, loc, all_results['EATT'], True)
        ...
        try:
            if first_build:
                ...
                models['FTcF_MDS'] = (clf, embedding)
            else:
                clf, embedding = models['FTcF_MDS']
            ...
            print('FTcF_MDS', e)
            all_results['coForest'] = compile_results(y_test, predicted, loc, all_results['FTcF_MDS'], True)
        ...
        try:
            if first_build:
                ...
                clf = models['coForest']
            predicted = clf.predict(X_test)
            all_results['coForest'] = compile_results(y_test, predicted, loc, all_results['coForest'], False)
        except Exception as e:
            print(model, e)
            all_results['coForest'] = compile_results(y_test, predicted, loc, all_results['coForest'], True)
        ...
        for model in supervised_model_list.keys():
            try:
                ...
                if first_build:
                    ...
                    models[boosting_model] = clf
                else:
                    clf = models[boosting_model]
                predicted = clf.predict(X_test)
                all_results[boosting_model] = compile_results(y_test, predicted, loc, all_results[boosting_model], False)
            except Exception as e:
                print(model, e)
                all_results[boosting_model] = compile_results(y_test, predicted, loc, all_results[boosting_model], True)
                continue
        ...
        try:
            if first_build:
                ...
                clf = models['S3VM']
            predicted = clf.predict(X_test)
            all_results['S3VM'] = compile_results(y_test, predicted, loc, all_results['S3VM'], False)
        except Exception as e:
            print(model, e)
            all_results['S3VM'] = compile_results(y_test, predicted, loc, all_results['S3VM'], True)
        print(X_test.shape)
        ...
        try:
            ...
            all_results['lapSvm'] = compile_results(y_test, predicted, loc, all_results['lapSvm'], False)
        except Exception as e:
            print('lapSvm', e)
            all_results['S3VM'] = compile_results(y_test, predicted, loc, all_results['lapSvm'], True)

    result_path = 'results/release_new/Project_specific_' + perc[percentage] + '/' + project + '.pkl'
    with open(result_path, 'wb') as handle:
        pkl.dump(all_results, handle, protocol=pkl.HIGHEST_PROTOCOL)

def run(projects, supervised_model_list, percentage):
    perc = {2.5:'2_5', 5: '5', 10: '10', 20: '20'}
    count = 0 
    for project in projects:
        result_path = 'results/release_new/Project_specific_' + perc[percentage] + '/' + project + '.pkl'
        file_exists = exists(result_path)
        if file_exists:
            continue
        else:
            try:
                run_classifiers(project, supervised_model_list, percentage)
            except Exception as e:
                print('exception:', e, project)
                continue
    print(count)
    return None
...
if __name__=="__main__":
    ...
    for project in existing_projects:
        result_path = 'results/Project_specific_' + perc[percentage] + '/' + project + '.pkl'
        file_exists = exists(result_path)
        if file_exists:
            continue
        else:
    ...
    existing_projects = final_projects
    ...
    for i in range(cores):
        ...
        t = ThreadWithReturnValue(target = run, args = [split_projects[i], supervised_model_list, percentage])
        threads.append(t)
